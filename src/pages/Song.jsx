...
function Song() {
  ...
  return (
    <div className="background">
        ...
        <h1 className="text-center titleword">{<LuHeadphones />} {songName} {<LuHeadphones />}</h1>
        <YoutubeEmbed embedId={youtube(songLink)} />
        <p className="dedication">{dedication}</p>
        <hr />
        <div className="d-grid gap-2">
            <button className="btn btn-pinky" onClick={handleGoHome}>Ir a la página de inicio</button>
            <button className="btn btn-pinky" onClick={handleGoToSongs}>Ver lista de canciones</button>
        </div>
        </div>
    </div>
  );
